...
interface AnnualProductionCurveProps {
  chartData: {
    byMonth: MonthData[]
...
export function AnnualProductionCurve({ chartData, isDarkMode }: AnnualProductionCurveProps) {
  ...
  const chartDataMerged = (() => {
    ...
    selectedYearsData.forEach((yearData) => {
      yearData.byMonth.forEach(monthData => {
        ...
        entry[`production_${yearData.label}`] = monthData.production
      })
    })
    ...
    <div className="space-y-4">
          ...
          >
            ...
            {selectedYearsData.map((yearData, index) => (
              <Line
                key={yearData.label}
                type="monotone"
                dataKey={`production_${yearData.label}`}
      ...
      <div className="p-4 bg-blue-50 dark:bg-blue-900/20 border border-blue-200 dark:border-blue-800 rounded-lg">
        <p className="text-sm text-blue-800 dark:text-blue-200">
          <strong>ℹ️ Note :</strong> Cette courbe présente votre production mensuelle sur des périodes glissantes de 365 jours. Vous pouvez sélectionner plusieurs périodes pour les comparer.
        </p>
      </div>
    </div>
  )
